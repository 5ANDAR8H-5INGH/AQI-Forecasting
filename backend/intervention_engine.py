...
# preventive/health-advisory measures commonly recommended by CPCB's
# National Air Quality Index guidance for the "Moderate" category, tagged
# by source category the same way GRAP_ACTIONS is so they can be
# prioritized against the current source attribution.
MODERATE_ADVISORY_ACTIONS = [
    {"source": "dust_construction",
     "action": "Continue routine (non-mandatory) water sprinkling and dust suppression at construction sites and unpaved roads as a preventive measure."},
    {"source": "vehicular",
     "action": "Public advisory encouraging carpooling/public transport during peak hours; continue routine PUC certificate checks."},
    {"source": "biomass_agri",
     "action": "Public messaging discouraging open burning of garbage or biomass, ahead of any formal enforcement stage."},
    {"source": "industrial",
     "action": "Routine compliance checks on industrial emission norms; no operational curbs required at this level."},
    {"source": "secondary_photochemical",
     "action": "Advise limiting strenuous outdoor activity during peak afternoon ozone hours."},
]

MODERATE_HEALTH_NOTE = ("Sensitive groups (children, older adults, and people with asthma or heart "
...
<<<<<<< HEAD
    if current_aqi <= 100:
        return {
            "stage": None,
            "note": "AQI is 'Good/Satisfactory' — no intervention or advisory needed. Continue routine monitoring.",
            "actions": [],
        }

    if current_aqi <= 200:
        priority_sources = {s["source"] for s in ranked_sources[:top_n_sources]}
        applicable = [a for a in MODERATE_ADVISORY_ACTIONS if a["source"] in priority_sources]
        if not applicable:
            applicable = MODERATE_ADVISORY_ACTIONS[:top_n_sources]
        return {
            "stage": "Advisory",
            "note": f"AQI {round(current_aqi)} is 'Moderate' — below GRAP's own trigger point (Poor, 201+), "
                    f"so no mandatory GRAP stage applies yet. Preventive/health-advisory actions below, "
                    f"prioritized toward the currently dominant suspected source categories. {MODERATE_HEALTH_NOTE}",
            "actions": [
                {
                    "stage": "Advisory",
                    "source": a["source"],
